import type { INDIAN_STATES } from '@/lib/india-states'

type IndianState = (typeof INDIAN_STATES)[number]

/**
 * Districts per state/UT, keyed by the exact names in INDIAN_STATES, for the
 * cascading State → District pickers in the address forms. Widely-used
 * district names; a shopper in a newly-carved district picks its parent.
 */
export const INDIA_DISTRICTS: Record<IndianState, string[]> = {
  'Andhra Pradesh': [
  ...
  Rajasthan: [
    ...
    'Jhunjhunu', 'Jodhpur', 'Karauli', 'Khairthal-Tijara',
    'Kotputli-Behror', 'Kota', 'Nagaur', 'Pali', 'Phalodi', 'Pratapgarh',
    'Rajsamand', 'Salumbar', 'Sawai Madhopur', 'Sikar', 'Sirohi',
...
/** The district options for a state, or [] for an unknown/empty state. */
export function districtsFor(state: string): string[] {
  return (INDIA_DISTRICTS as Record<string, string[]>)[state] ?? []
}
